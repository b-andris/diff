...
diffComponent* makeDiffComponentForUnchanged(char* c) {
	diffComponent* d = malloc(sizeof(diffComponent));
	d->from = d->to = c;
	return d;
}

diffComponent* makeDiffComponent(char* from, char* to) {
	diffComponent* d = malloc(sizeof(diffComponent));
	d->from = from;
	d->to = to;
	return d;
...
char* stringByAppendingChar(char* oldStr, char c){
	unsigned long strLen = strlen(oldStr);
	char* str = realloc(oldStr, (strLen + 2) * sizeof(char));
	str[strLen] = c;
	str[strLen + 1] = '\0';
	return str;
}
...
diffComponent** internalDiff(const char* from, const char* to, int* cache, unsigned long fromOffset, unsigned long toOffset, unsigned long fromLength, unsigned long toLength, int* componentCount) {
	...
	if (from[fromIndex] == to[toIndex]) {
		...
		return appendUnchangedCharToBacktrack(from[fromIndex], prevBackTrack, componentCount);
	}
